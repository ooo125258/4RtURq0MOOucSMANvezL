...
def main(argv, prog=''):
    ...
    '----------------------------------\nTimings\n----------------------------------'
    print
    'Reading:    %g seconds' % ((t2 - t1) / cv.getTickFrequency())
    print
    'Processing: %g seconds' % ((t3 - t2) / cv.getTickFrequency())
    print
    'Writing:    %g seconds' % ((t4 - t3) / cv.getTickFrequency())

    # return any command-line arguments that were not processed
    return success, unprocessedArgv

def runMatting(arg1, arg2):
    main(arg1, arg2)
