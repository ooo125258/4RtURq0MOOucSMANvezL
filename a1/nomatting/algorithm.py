...
class Matting:
    ...
    def readImage(self, fileName, key):##TODO: Assume it's a colorful image, all pictures same size
        ...
        if loader is None:
            ...
            #loader = cv.cvtColor(loader, cv.COLOR_BGR2RGB)
...
success, errorMessage = triangulationMatting(self)
        ...
        deltaC = np.concatenate((self._images['compA'] - self._images['backA'], self._images['compB'] - self._images['backB']), axis = 2)
        I = np.identity(3, dtype=int)
        matrix = np.concatenate((I,I), axis=0)
        try:
            colOut = np.zeros(self._images['backA'].shape, dtype=int)
            alphaOut = np.zeros(self._images['backA'].shape[:2], dtype=int)
            for i in range(colOut.shape[0]):
                for j in range(colOut.shape[1]):
                    ...
                    #(C.T * C)^-1 * C^T * d = F
            #########################################TODO: alpha is negative. I need to check the reason
                    Fprime = np.dot(np.linalg.inv(np.dot(coeff_Fprime.T, coeff_Fprime)), np.dot(coeff_Fprime.T, deltaC_ij))
                    colOut[i, j] = Fprime[:3] / Fprime[-1]
